...
from classification_label_to_image import overlayonorginal
...
# x_test=[] # data variable holder
# selected_sample_per_class=15 *1000 # number of patches for downsampling/upsampling  MAX/ MIN ( )
...
# selected_sample_per_class_train= int(selected_sample_per_class*trainPercent)
# selected_sample_per_class_test = int(selected_sample_per_class*testPercent)
...
def multi_gpu_cnn_model(dimension, numChannels,numOfClasses):
    model = Sequential()
...
def saveModelResultsToText(history, directory):
    keys =list(history.history.keys())
    file = open(directory + 'model_results.txt', 'w+')
    s = f'Number of epochs   : {history.epoch[-1] + 1}\n' \
        f'Traning Accuracy   : {history.history[keys[3]][-1]}\n' \
        f'Traning Loss       : {history.history[keys[2]][-1]}\n' \
        f'Validation Accuracy: {history.history[keys[1]][-1]}\n' \
        f'Validation Loss    : {history.history[keys[0]][-1]}\n'
    file.write(s)
    file.close()

def run_cnn(dimension =3,
            deleted_channels=[],
            selected_sample_per_class= 15*1000,
            balanced_option='balanced',
            epochs=200,
            batchSize=256,
            numOfClasses = 5,
            testPercent =0.1):
    numChannels = 7-len(deleted_channels)
    labels = []  # data variable holder
    labels_test = []
    x_train = []  # data variable holder
    x_test = []  # data variable holder
    ...
    plt.ioff() # turning off plt display
    # call the CNN model
    multi_model = multi_gpu_cnn_model(dimension,numChannels,numOfClasses) #intialize CNN model parameters
    ...
    save_directory = './global_model_3_2_modified/'+'Single_classifier_del_chl_'+str(deleted_channels)+'_epochs_'+str(epochs)+'_dim_'+str(dimension)+'_spc_'+str(selected_sample_per_class)+'_batchS_'+str(batchSize)+'/'
    ...
    if os.path.isfile(save_directory+cnnFileName):
        ...
        for class_numb in range(0, len(class_name)):
            ...
            for iter_locat in range(0, len(locations)):
                ...
                sample_data_test = [] # variable holder declared
                ...
                #split the sample data for traning and testing before up/down sampeling them or(balencing)
                sample_data,sample_data_test =np.split(sample_data,[int(len(sample_data)*trainPercent)])
                ...
                if balanced_option == 'balanced':
                    ...
                    if sample_data.shape[0] > selected_sample_per_class:
                        index_downsample_train = np.random.choice(sample_data.shape[0], selected_sample_per_class_train, replace=False)
                        index_downsample_test = np.random.choice(sample_data_test.shape[0], selected_sample_per_class_test, replace=False)
                    elif sample_data.shape[0] < selected_sample_per_class:
                        index_downsample_train = np.random.choice(sample_data.shape[0], selected_sample_per_class_train)
                        index_downsample_test = np.random.choice(sample_data_test.shape[0], selected_sample_per_class_test)
                    sample_data = sample_data[index_downsample_train, :, :, :]
                    sample_data_test = sample_data_test[index_downsample_test, :, :, :]
                    # print('after blancing Sample data Shape', sample_data.shape)
                    # print(f' pathnum :{patchnumb}, ,,, len(a):{len(A)-1}')

                    labels.append(class_numb * np.ones(selected_sample_per_class_train))
                    labels_test.append(class_numb * np.ones(selected_sample_per_class_test))
                    # print(f'After balencing-> sample_data_train_shape: {sample_data.shape} |sample_data_test_shape: {sample_data_test.shape}')
                    x_train.append(sample_data)
                    x_test.append(sample_data_test)
                    del sample_data
                elif balanced_option == 'unbalanced':
                    # unbalanced dataset
                    labels.append(class_numb * np.ones(int(sample_data.shape[0])))
                    labels_test.append(class_numb * np.ones(int(sample_data_test.shape[0])))
                    x_train.append(sample_data)
                    x_train.append(sample_data)
                    del sample_data
        # concatenate all the training & testing dataset
        ...
        x_test = np.delete(x_test, deleted_channels, 3)
        print(x_train.shape, labels.shape)
        y_train = np_utils.to_categorical(labels) # one hot vector conversion
        y_test = np_utils.to_categorical(labels_test) # one hot vector conversion
        ...
        print(f'traning data: {x_train.shape} \t validation data: {x_test.shape}')
        history=multi_model.fit(x_train, y_train, validation_data=(x_test,y_test),  epochs=epochs, batch_size=batchSize, shuffle=True, verbose=0, callbacks=[TqdmCallback(verbose=0)])
        saveModelResultsToText(history,save_directory)
        # print(history)
        ...
        plt.clf()
        plt.plot(history.history['accuracy'],linewidth=0.8)
        plt.plot(history.history['val_accuracy'],linewidth=0.8)
        plt.title('model accuracy')
        plt.ylabel('accuracy')
        plt.xlabel('epoch')
        plt.legend(['train', 'validation'])
        plt.savefig(save_directory+'acc.png',dpi=600)
        plt.clf()
        # "Loss" curve save in png format for training and validation
        plt.plot(history.history['loss'],linewidth=0.8)
        plt.plot(history.history['val_loss'],linewidth=0.8)
        plt.title('model loss')
        plt.ylabel('loss')
        plt.xlabel('epoch')
        plt.legend(['train', 'validation'])
        plt.savefig(save_directory+'loss.png',dpi=600)
    ...
    for iter_locat in range(0, len(locations)):
        ...
        blendimg = overlayonorginal(im3,save_directory+location+'_' + method_name[0:-4])
        cv2.imwrite(save_directory+'10Oct13_10Oct13_overlayed.png',blendimg)
        difference = datetime.now() - start
        print(difference)
        difference = datetime.now() - start
        print(difference); # calculate the time
